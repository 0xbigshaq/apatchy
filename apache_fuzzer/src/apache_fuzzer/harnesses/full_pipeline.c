/*
 * @description: Full Apache pipeline harness - injects input via custom bucket/filters
 *
 * This harness reads HTTP requests from stdin and processes them through
 * Apache's full request handling pipeline without using network sockets.
 *
 * The approach:
 ...
 * 4. Create a fake connection and process it
 */
...
/*
 * Process one fuzz input - used by both standalone and libfuzzer modes
 */
